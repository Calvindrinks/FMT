...
def play(args):
    ...
    cur_contact_sum = torch.zeros(env.num_envs, dtype=torch.float, device=env.device)
    ...
    for i in range(1*int(env.max_episode_length)):
        ...
        # actions.fill_(0)
        hurt_contact = env.contact_forces[:, env.feet_indices[0], 2] > 1.
        if not torch.all(hurt_contact==env.last_contacts[:,0]):
            print("This time don't same")
        obs, _, rews, dones, infos = env.step(actions.detach())
        # cur_contact_sum += env.last_contacts[:, 0] * env.dt
        contact_filt = torch.logical_or(hurt_contact, env.last_contacts[:, 0]) 
        cur_contact_sum += contact_filt * env.dt
        if RECORD_FRAMES:
            if i % 2:
        ...
        if MOVE_CAMERA:
            ...
            camera_actor = env.camera_actor * (env.num_envs // 4)
            root_state = env.root_states[camera_actor, :3].cpu().numpy()
    ...
    if DRAW_SURVIVE:
        ...
        print(f"reward_buf, what reward{torch.max(cur_contact_sum)}")
        
        for i in range(groups_num // commands_group_num):
            group_reward = torch.sum(cur_contact_sum[commands_group==i]) / (env.num_envs // groups_num)
        ...
        folder_name = "contact"
        # folder_name = "contact_without_filter"
        if args.mm and DISCRIMINATOR:
            if args.load_run == "newbest":
                name = f"./legged_gym/tests/pictures/{folder_name}/con_{FAULT_TYPE}_ours.pt"
            elif args.load_run == "no_Reflect":
                name = f"./legged_gym/tests/pictures/{folder_name}/con_{FAULT_TYPE}_wo_reflect.pt"
        elif args.mm and not DISCRIMINATOR:
            name = f"./legged_gym/tests/pictures/{folder_name}/con_{FAULT_TYPE}_taskpolicy.pt"
        else:
            if args.load_run == "mix":
                name = f"./legged_gym/tests/pictures/{folder_name}/con{FAULT_TYPE}_mixTrain_normal.pt"
            else:
                name = f"./legged_gym/tests/pictures/{folder_name}/con_{FAULT_TYPE}_normal.pt"
